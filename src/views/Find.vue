    ...
    <div class="share">
        <div class="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-white border-bottom shadow-sm">
            ...
            <nav class="my-2 my-md-0 mr-md-3">
                ...
                <span v-else>
                    <router-link class="p-2 text-dark" :to="{name:'login'}">登录</router-link>
                    <router-link class="btn btn-outline-primary" :to="{name:'register'}">注册</router-link>
                </span>
            </nav>
        </div>
                                        ...
                                        <div class="media align-items-center first-field-style">
                                            <div v-if="row.item.isDir">
                                                ...
                                                <span class="name mb-0 text-sm">
                                                    <a :href="website + row.item.fsk">{{row.value}}</a>
                                                    </span>
                                            </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    {key: 'fsk', label: '共享码'},
                    {key: 'uptime', label: '上传时间', class: 'text-center'},
                ],
        ...
        created() {
            ...
            //存储 sid
            this.args = this.$route.params.args;
            //获取文件列表
            this.$ajax
                .get(this.server + "/api/general/share/user?args=" + this.args)
                .then(response => {
                    this.items.push.apply(this.items, response.data.data);
